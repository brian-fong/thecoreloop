const PROJECTS: any[] = [
  {
    name: "Sipher Odyssey",
    studio: "Ather Labs",
    submittedAs: "Community",
    date: "Mar 15 2023",
    thumbnail: "https://pbs.twimg.com/profile_images/1630762842273247239/WUWeosk9_400x400.jpg",
    blockchain: "Polygon",
    tagline: "Sipher is a multiplayer looter shooter ARPG, set in a dream-like universe of alien worlds, vibrant characters, and dangerous mysteries.",
    description: "Sipherian Odyssey lets players buy, trade, acquire, and sell commodities and items as a core element of its gameplay. Each of Sipheria's multiple worlds has a unique distribution of limited resources, all of which can be gathered and utilized in deep crafting systems.",
    stage: "Production",
    fundraising: "undisclosed",
    upvotes: 123,
    submitter: {
      name: "cloud",
      image: "https://pbs.twimg.com/profile_images/1635117890440695808/j3Ww7-z7_400x400.jpg",
    },
    genres: ["Action", "Shooter"],
    links: [
      "https://sipher.xyz/",
      "https://twitter.com/playSIPHER",
      "https://discord.com/invite/playSIPHER",
      "https://opensea.io/collection/sipheriansurge",
    ],
    gallery: [
      "https://i.imgur.com/pB5jaXX.jpg",
      "https://i.imgur.com/2p8YQBm.jpg",
      "https://i.imgur.com/mEqfh5Q.jpg",
      "https://i.imgur.com/zVWsePi.jpg",
    ],
    story: `Hey TCL community, super excited to share with you guys about Sipher and why I’ve been bullish since their initial mint.

Sipher will be creating a compelling game world built for gamers by gamers, around the core philosophy of "casual to start, fun to play, hard to master”. While the actual gameplay details are not finalized, there are early screens of a Multiplayer Online Battle Arena gameplay as well as a co-op dungeon crawler.

The founder - Tin Nguyen, is an avid & passionate gamer founder who aptly takes on the role of Sipheria’s World Architect. Personally designing the game art and writing lore, he takes great care to bring to life his vision of an immersive world built for players to adventure. Along with this passion for NFT gaming and the metaverse, he understands that a fun game, lucrative incentives and consistent content releases are key to drive adoption and a sustainable game economy.

From the top-quality game graphics, and deep world lore to the community management skills on display, I have full confidence in the execution of the team and I personally can’t wait to experience the game!`
  },
  {
    name: "Legions & Legends",
    studio: "Azra Games",
    submittedAs: "Community",
    date: "Apr 23 2023",
    thumbnail: "https://pbs.twimg.com/profile_images/1631055277003816962/W4fAXTn3_400x400.jpg",
    blockchain: "Ethereum",
    tagline: "A New Sci-fi / Fantasy Action RPG from the Creators of Star Wars: Galaxy of Heroes",
    description: "Legions & Legends is a collectible and combat RPG, set in an uncharted galaxy where magic and technology collide. Here, powerful civilizations, as yet unknown to Earth, are embroiled in an eternal war.",
    stage: "Production",
    fundraising: "no",
    upvotes: 51,
    submitter: {
      name: "0xKapitalK",
      image: "https://pbs.twimg.com/profile_images/1551347350618025984/Ica0XNb-_400x400.jpg",
    },
    genres: ["Action", "RPG"],
    links: [
      "https://legionsandlegends.com/",
      "https://twitter.com/legions_legends",
      "https://discord.com/invite/azra",
    ],
    gallery: [
      "https://i.imgur.com/IHjdiwy.jpg",
      "https://i.imgur.com/B9MYu4F.jpg",
      "https://i.imgur.com/tQaBwS2.jpg",
      "https://i.imgur.com/x2XD3d7.jpg",
      "https://i.imgur.com/XEaDpCv.jpg",
    ],
    story: `Hey TCL community, 0xKapitalK here! Been a big fan of what Mark Otero and his team have been building for awhile and wanted to share with you my personal thoughts on why I’m excited for Legions & Legends under Azra Games.
    
    High Probability for Success. This is Otero’s 9th Collect & Combat RPG — this signals low execution risk in terms of game development and delivery.   As an ex-hedge fund professional, he understands markets — more particularly the unrelenting and brutal nature of markets.   Additionally, the team has worked together successfully before in the past.  This is a key element that a lot of other projects lack.
    
    Founder-Genre Fit. Mark grew up playing the correct games and therefore was influenced by the appropriate ideals and values that align perfectly with crypto.  As a D&D cult player, he not only understood but experienced the power of true ownership through physically — and emotionally — holding onto his own character sheet.  
    
    Genre fit. Azra Games operates in a unique sliver of the RPG genre game design that matches the best with blockchain. Ownership is part of the product and gameplay experience.
    
    Narrative & Storytelling. The key element to a game is the story.  Neel Upadhye is a world builder who was written & directed award-winning short films with world-class IPs like StarWars and Apex Legends.  He was also the Narrative lead for Battlefield 2042 and lead writer for Need for Speed.
    
    With all that being said, Legions & Legends is certainly one to look out for in the next era of web3 games`
  },
  {
    name: "Paradise Tycoon",
    studio: "Empires Not Vampires",
    submittedAs: "Community",
    date: "April 21 2023",
    thumbnail: "https://pbs.twimg.com/profile_images/1493583534157611010/_FJb-Rg6_400x400.png",
    blockchain: "Avalanche",
    tagline: "Build your Paradise, Have fun with your Friends and Compete to become the Ultimate Tycoon in our immersive Multiplayer",
    description: "Collaborate with your neighboring landowners in events and create a lush utopian community. Invite friends across the metaverse for a visit to unwind in the harmonious environment of your paradise or hang out with the diverse cast of amazing characters at Port Ohana! Experience your very own paradise in the most exciting web3 game of 2023.",
    stage: "Playable Demo",
    fundraising: "undisclosed",
    upvotes: 91,
    submitter: {
      name: "0xconglomerate",
      image: "https://pbs.twimg.com/profile_images/1638051849394868224/xBWabna5_400x400.jpg",
    },
    genres: ["Sandbox", "Simulation"],
    links: [
      "https://paradisetycoon.com/",
      "https://paradisetycoon.com/play/",
      "https://twitter.com/ParadiseTycoon",
      "https://discord.com/invite/paradisetycoon",
    ],
    gallery: [
      "https://i.imgur.com/ym1zWJ2.jpg",
      "https://i.imgur.com/iv3hYf5.jpg",
      "https://i.imgur.com/fIlmZKE.jpg",
      "https://i.imgur.com/PTdGolp.jpg",
    ],
    comments: [
      {
        user: {
          name: "0xconglomerate",
          image: "https://pbs.twimg.com/profile_images/1638051849394868224/xBWabna5_400x400.jpg",
        },
        tags: "Submitter",
        date: "April 21 2023",
        content: 
          `Hey everyone, NN here! We invested in Paradise Tycoon early on and wanted to share more with the TCL community more about Paradise Tycoon!

          In short, Paradise Tycoon is a web3 social simulation game in an MMO world. Players can gather resources, explore, build & craft items for their own use or trade. Players can buy land & upgrade them.

          The strong feature here imo is that people can explore the paradise & uninhabited island solo or with friends. They can travel across the metaverse and level up their avatars. Haven’t seen many web3 games implementing this.

          It can be compared to Animal Crossing which had over 38M downloads. In general, I’m a fan of social simulation games cuz its for all ages. In general, it’s a trending game genre (3rd most popular).

          To try out the game urself, head over to https://paradisetycoon.com/play.`,
      },
    ]
  }
];

export default PROJECTS;

