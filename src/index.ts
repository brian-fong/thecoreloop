...
import writeIndex from "./scripts/write-telegram-index";
...
async function main(): Promise<void> {
  ...
  const main_menu: Choice[] = [
    ...
    { 
      name: "Write Telegram Index for thecoreloop_test Channel",
      value: 4,
    },
  ];
  ...
  for (const user_choice of main_input) {
    ...
    if (user_choice == 0) {
      ...
      for (const filename_LAG_json of filenames_LAG_json) {
        try {
          ...
          if (lag.number <= latest_LAG_number) {
            ...
            await sendMessage(client, "thecoreloop_test", formatString(lag));
      ...
      const messages_lag_test: TelegramMessage[] = await readMessages(client, "thecoreloop_test", number_array);
      plog.done(`Done`, 0, 1);
      plog.log(`==> ${messages_lag_test.length} messages found`, 0, 1);

      // Parse LAG posts
      const messages_thecoreloop_test: string[] = [];
      for (const message of messages_lag_test) {
        try {
          const lag: LAG = new LAG(message);
      ...
      if (messages_lag.length != messages_lag_test.length) throw Error("Channels contain differing number of LAG posts");

      // Compare messages
      const messages_delta: string[] = [];
      plog.log(`Comparing messages . . . `, 0, 0);
      for (let i = 0; i < messages_lag.length; i++) {
        const message_a: TelegramMessage = messages_lag[i];
        const message_b: TelegramMessage = messages_lag_test[i];
      ...
      // === Write Telegram Index for thecoreloop_test Channel === 
      
      await writeIndex(client, "thecoreloop_test");
    }
  }
}
